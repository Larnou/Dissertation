...
def _resolve_excluded_range(
    excluded_range: Sequence[float] | None,
    h_value: float | None,
    delta: float | None,
) -> tuple[float, float]:
    if excluded_range is not None:
        if len(excluded_range) != 2:
            raise ValueError("excluded_range must contain exactly two values: [lower, upper].")
        lower = float(excluded_range[0])
        upper = float(excluded_range[1])
    else:
        if h_value is None or delta is None:
            raise ValueError("Either excluded_range or both h_value and delta must be provided.")
        if delta < 0:
    ...
    if not np.isfinite(lower) or not np.isfinite(upper):
        raise ValueError("Excluded range bounds must be finite numbers.")
    if upper < lower:
        raise ValueError("Excluded range upper bound must be greater than or equal to lower bound.")
...
def _prepare_ae_histogram(
    ...
    if log_scale:
        ...
        return values, counts, bin_edges, bin_centers
...
def plot_ae_histogram(
    ...
    ae_values, counts, bin_edges, bin_centers = _prepare_ae_histogram(
    ...
    ax.set_title("AE index distribution" + (" (log scale)" if log_scale else ""), fontsize=14)
    ...
    resolved_output = Path(output_path) if output_path else Path("hist_ae_index_log.png" if log_scale else "hist_ae_index.png")
...
def plot_ae_histogram_outside_h_range(
    ...
    output_path: str | Path | None = None,
    show: bool = True,
    excluded_range: Sequence[float] | None = None,
) -> Path:
    ...
    lower, upper = _resolve_excluded_range(excluded_range, h_value, delta)
    ...
    ax.set_title(f"AE distribution outside {h_column} in [{lower:.3g}, {upper:.3g}]" + (" (log scale)" if log_scale else ""), fontsize=14)
    ax.set_xlabel("AE index" + (" (log scale)" if log_scale else ""), fontsize=12)
    ax.set_ylabel("Counts", fontsize=12)
    ax.grid(alpha=0.35)
    ax.yaxis.set_minor_locator(AutoMinorLocator(5))
    ...
    resolved_output = Path(output_path) if output_path else Path(f"hist_ae_outside_H_{normalized_component}_{lower:g}_{upper:g}{suffix}.png")
    if output_path is not None:
        resolved_output.parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(resolved_output, dpi=300)
