...
class IonCurrentDensityModel:
    """
    Расчёт плотности ионного тока по компонентам:
        J_* = q * n_p * V_*

    где:
    - q: заряд иона (по умолчанию +e),
    - n_p: концентрация из столбца ``density``,
    - V_*: компонентa средней скорости (обычно ``V_a``, ``V_r``, ``V_f``).
    """
    ...
    def model(self) -> pd.DataFrame:
        """
        Возвращает DataFrame со столбцами ``J_a``, ``J_r``, ``J_f`` в А/м².
        """
