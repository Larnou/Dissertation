...
def save_data_to_parquet(config: AppConfig, dataframe: pd.DataFrame, dataset_stem: str, title: bool = False) -> None:
    """
    Сохраняет DataFrame в parquet под `backend/data/events/...`.

    Параметр `title` оставлен для совместимости: при True индекс пишется в файл.
    """
    path = parquet_dataset_path(config, dataset_stem)
    path.parent.mkdir(parents=True, exist_ok=True)
    dataframe.to_parquet(path, index=title)
